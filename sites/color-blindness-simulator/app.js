 /* istanbul ignore next */ const TYPES=[{name:'Normal Vision',matrix:[1,0,0,0,1,0,0,0,1]},{name:'Protanopia',matrix:[.567,.433,0,.558,.442,0,0,.242,.758]},{name:'Deuteranopia',matrix:[.625,.375,0,.7,.3,0,0,.3,.7]},{name:'Tritanopia',matrix:[.95,.05,0,0,.433,.567,0,.475,.525]},{name:'Achromatopsia',matrix:[.299,.587,.114,.299,.587,.114,.299,.587,.114]}];
  /* istanbul ignore next */ function hexToRgb(hex){return[parseInt(hex.slice(1,3),16),parseInt(hex.slice(3,5),16),parseInt(hex.slice(5,7),16)]}
 function rgbToHex(r,g,b){return'#'+[r,g,b].map(c=>Math.round(Math.max(0,Math.min(255,c))).toString(16).padStart(2,'0')).join('')}
  /* istanbul ignore next */ function applyMatrix(rgb,m){return[m[0]*rgb[0]+m[1]*rgb[1]+m[2]*rgb[2],m[3]*rgb[0]+m[4]*rgb[1]+m[5]*rgb[2],m[6]*rgb[0]+m[7]*rgb[1]+m[8]*rgb[2]]}
...
el.innerHTML='<div class="card glass"><h3>Pick a Color</h3><input type="color" id="color-pick" value="'+hex+'" style="width:100%;height:60px;border:none;cursor:pointer" oninput="simulate()"><div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(180px,1fr));gap:12px;margin-top:16px">'+cards+'</div></div>'}
 ...
 /* istanbul ignore next */ if(typeof document!=='undefined')document.addEventListener('DOMContentLoaded',simulate);

 /* istanbul ignore next */ if(typeof module!=='undefined'&&module.exports)module.exports={TYPES,hexToRgb,rgbToHex,applyMatrix,simulate};
