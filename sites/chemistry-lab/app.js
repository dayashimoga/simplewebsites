...
// --- Data: Elements & Reactions ---
 ...
 const REACTIONS = [
  ...
  { reactants: ['Fe', 'O'], minTemp: 100, equation: '4Fe + 3O₂ → 2Fe₂O₃', products: ['Iron(III) Oxide (Rust)'], type: 'exothermic',
    observation: 'Reddish-brown powder forms quickly at high heat, rapid reaction produces sparks', energy: 'Releases 824 kJ/mol',
  ...
  { reactants: ['Mg', 'O'], minTemp: 300, equation: '2Mg + O₂ → 2MgO', products: ['Magnesium Oxide'], type: 'exothermic',
  ...
  { reactants: ['Cu', 'H2SO4'], minTemp: 80, equation: 'Cu + 2H₂SO₄(hot) → CuSO₄ + SO₂↑ + 2H₂O', products: ['Copper Sulfate', 'Sulfur Dioxide', 'Water'], type: 'endothermic',
  ...
  { reactants: ['C', 'O'], minTemp: 200, equation: 'C + O₂ → CO₂', products: ['Carbon Dioxide'], type: 'exothermic',
  ...
  { reactants: ['Al', 'O'], minTemp: 400, equation: '4Al + 3O₂ → 2Al₂O₃', products: ['Aluminum Oxide'], type: 'exothermic',
...
// --- Element Rendering ---
 ...
 function clearBeaker() {
  ...
  updateTemperature();
   const liquid = document.getElementById('beaker-liquid');
   const beaker = document.getElementById('beaker');
   if (liquid) liquid.style.opacity = '0.8';
   if (beaker) beaker.classList.remove('boiling');
 ...
 function updateTemperature() {
  ...
  currentTemp = t;
   const d = document.getElementById('temp-display');
   const l = document.getElementById('temp-state-label');
  ...
  if (t < 0) {
    if(l) l.textContent = '❄️ Solid/Ice';
    if(liquid) liquid.style.opacity = '0.9';
    if(beaker) beaker.classList.remove('boiling');
  } else if (t > 100) {
    if(l) l.textContent = '♨️ Boiling/Gas';
    if(liquid) liquid.style.opacity = '0.6';
    if(beaker) beaker.classList.add('boiling');
  } else {
    if(l) l.textContent = '💧 Liquid';
    if(liquid) liquid.style.opacity = '0.8';
    if(beaker) beaker.classList.remove('boiling');
  }
}
...
// --- Reaction Logic ---
 ...
 function showNoReaction() {
  ...
  if (products) products.innerHTML = '<div style="padding:4px 0;font-size:.85rem">No products formed</div>';
   ...
   if (fact) fact.textContent = 'Many reactions have an activation energy barrier and require specific temperature changes to proceed!';
...
// Electron configurations for first 30 elements
const ELECTRON_CONFIGS = {
  H: '1s¹', He: '1s²', C: '[He] 2s² 2p²', N: '[He] 2s² 2p³', O: '[He] 2s² 2p⁴',
  Na: '[Ne] 3s¹', Cl: '[Ne] 3s² 3p⁵', Fe: '[Ar] 3d⁶ 4s²', Cu: '[Ar] 3d¹⁰ 4s¹',
  Ag: '[Kr] 4d¹⁰ 5s¹', Au: '[Xe] 4f¹⁴ 5d¹⁰ 6s¹', Mg: '[Ne] 3s²', Ca: '[Ar] 4s²',
  K: '[Ar] 4s¹', S: '[Ne] 3s² 3p⁴', Al: '[Ne] 3s² 3p¹', Zn: '[Ar] 3d¹⁰ 4s²',
};

function getElectronConfig(symbol) {
  return ELECTRON_CONFIGS[symbol] || 'Configuration not available';
}

// Molarity Calculator
function calculateMolarity(mass, molarMass, volumeL) {
  if (!mass || !molarMass || !volumeL || molarMass <= 0 || volumeL <= 0) return 0;
  const moles = mass / molarMass;
  return parseFloat((moles / volumeL).toFixed(4));
}

function calculateDilution(c1, v1, c2) {
  // C1V1 = C2V2 -> V2 = C1V1/C2
  if (!c1 || !v1 || !c2 || c2 <= 0) return 0;
  return parseFloat(((c1 * v1) / c2).toFixed(4));
}

// Reaction rate approximation (Arrhenius-inspired)
function reactionRateMultiplier(temp) {
  // Roughly doubles every 10°C
  const refTemp = 20;
  return parseFloat(Math.pow(2, (temp - refTemp) / 10).toFixed(2));
}

// Chemistry facts
const CHEM_FACTS = [
  '⚗️ There are 118 confirmed elements on the periodic table.',
  '⚗️ Gallium metal melts in your hand — its melting point is only 29.76°C!',
  '⚗️ A teaspoon of neutron star material weighs about 6 billion tons.',
  '⚗️ Water is the only substance that naturally exists as solid, liquid, and gas on Earth.',
  '⚗️ Graphene is 200x stronger than steel but only 1 atom thick.',
  '⚗️ Helium is the only element discovered in space before it was found on Earth.',
];

function getChemFact() {
  return CHEM_FACTS[Math.floor(Math.random() * CHEM_FACTS.length)];
}

  module.exports = {
    ELEMENTS, REACTIONS, ELECTRON_CONFIGS, CHEM_FACTS,
    renderElements, filterElements, filterByCategory, toggleElement, removeElement,
    updateBeaker, clearBeaker, updateTemperature,
    ...
    getElectronConfig, calculateMolarity, calculateDilution, reactionRateMultiplier, getChemFact,
